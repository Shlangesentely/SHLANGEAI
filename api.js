...
const AuthAPI = {
    /**
     * Login with admin code and get JWT token
     * @param {string} adminCode - Admin code
     * @returns {Promise<{token: string, expiresAt: string}>} Token and expiration
     */
    async login(adminCode) {
        ...
        try {
            ...
            Storage.setAdminToken(data.token, data.expiresAt);
    ...
    logout() {
        ...
        return Storage.clearAdminToken();
    },
    ...
    getAuthHeader() {
        ...
        if (Storage.isAdminTokenExpired()) {
            console.warn('[AuthAPI] Token is expired');
    ...
    isAuthenticated() {
        const token = Storage.getAdminToken();
        return token && !Storage.isAdminTokenExpired();
    }
};

// Make API and AuthAPI available globally
window.API = API;
